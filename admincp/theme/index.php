...
?>
			...
			<ul class="quickStats">
<?PHP
$Users_online = mssql_query("SELECT count(*) FROM ".$mvcore_medb_s." WHERE ConnectStat = '1'");
$Users_online_drop =mssql_result($Users_online, 0, 0);

$Total_Users = mssql_query("SELECT count(*) FROM ".$mvcore_medb_i."");
$Total_Users_drop =mssql_result($Total_Users, 0, 0);

$Total_Characters = mssql_query("SELECT count(*) FROM Character");
$Total_Characters_drop =mssql_result($Total_Characters, 0, 0);

$Total_charsban = mssql_query("SELECT count(*) FROM Character where CtlCode = '1'");
$Total_Charsban_drop =mssql_result($Total_charsban, 0, 0);

$Total_accban = mssql_query("SELECT count(*) FROM ".$mvcore_medb_i." where bloc_code = '1'");
$Total_accban_drop =mssql_result($Total_accban, 0, 0);

$Total_Guilds = mssql_query("SELECT count(*) FROM Guild");
$Total_Guilds_drop =mssql_result($Total_Guilds, 0, 0);
