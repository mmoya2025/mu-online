...
if (isset($_POST['change_user_pass'])){
	...
	if($old_pass == '') { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_changepassfail.'</div>'; } else {
				...
				$check_user_existd = mssql_query("Select memb___id from ".$mvcore_medb_i." where memb___id ='".$_SESSION['username']."'"); 
				$check_user_existssfaw = mssql_fetch_row($check_user_existd);

				if($mvcore['md5_support'] == 'yes') {
					if($mvcore['md5_hash_o'] == 'pw') { $pass_hash = hash('md5', $old_pass); } 
					elseif($mvcore['md5_hash_o'] == 'pw_ur') { $pass_hash = hash('md5', $old_pass.$check_user_existssfaw[0]); } 
					elseif($mvcore['md5_hash_o'] == 'ur_pw') { $pass_hash = hash('md5', $check_user_existssfaw[0].$old_pass); } 
					elseif($mvcore['md5_hash_o'] == 'pw_sal') { $pass_hash = hash('md5', $old_pass.$mvcore['md5_hs_salt']); }
					elseif($mvcore['md5_hash_o'] == 'pw_ur_sal') { $pass_hash = hash('md5', $old_pass.$check_user_existssfaw[0].$mvcore['md5_hs_salt']); } 
					elseif($mvcore['md5_hash_o'] == 'ur_pw_sal') { $pass_hash = hash('md5', $check_user_existssfaw[0].$old_pass.$mvcore['md5_hs_salt']); }
					elseif($mvcore['md5_hash_o'] == 'dbo_fn_md5') { $pass_hash = "dbo.fn_md5('".$old_pass."','".$check_user_existssfaw[0]."')"; }
					else { $pass_hash = hash('md5', $old_pass); } 
				};
				
				if($mvcore['md5_support'] == 'yes') {
					$check_user_exist = mssql_query("SELECT memb__pwd, memb___id, mvc_vip_date,smtp_block from ".$nmedb_data." where memb___id = '".$_SESSION['username']."' and memb__pwd = ".$pass_hash.""); 
					$check_user_existss = mssql_fetch_row($check_user_exist);
					if($check_user_existss[0] != ''){ } else { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_cannotnotfound.'</div>'; };
				} else {
					$check_user_exist = mssql_query("Select memb__pwd from ".$mvcore_medb_i." where memb___id ='".$_SESSION['username']."'"); 
					$check_user_existss = mssql_fetch_row($check_user_exist);
					if($check_user_existss[0] == $old_pass){ } else { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_cannotnotfound.'</div>'; };
				};
				
				if($old_pass == '') { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_oldpassforgot.'</div>'; };
				if($new_pass == '') { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_accnewpassmissing.'</div>'; };
				if($new_pass != $rep_pass) { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_passdidnotrepeat.'</div>'; };
				
				if(strlen($old_pass) >= '11') { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_oldpasstolarge.'</div>'; };
				if(strlen($new_pass) >= '11') { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_newpasstolarge.'</div>'; };
				if(strlen($rep_pass) >= '11') { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_account_settings_reppasstolarge.'</div>'; };
				
			if($has_error >= '1'){} else {
				
				if($mvcore['md5_support'] == 'yes') {
					$do_reg_insert = mssql_query("update ".$mvcore_medb_i." set memb__pwd = 0x".md5($user_pass)." where memb___id = '".$_SESSION['username']."'");
				} else {
					$do_reg_insert = mssql_query("update ".$mvcore_medb_i." set memb__pwd = '".$new_pass."' where memb___id = '".$_SESSION['username']."'");
				}
...
if (isset($_POST['change_user_country'])){
	...
	$mvc_flags 	= $_POST['ss_country'];
		...
		if($has_error >= '1'){} else {
			...
			$do_reg_insert = mssql_query("update ".$mvcore_medb_i." set mvc_flag = '".$mvc_flags."' where memb___id = '".$_SESSION['username']."'");
			echo'<div class="mvcore-nNote mvcore-nSuccess">Your country successfully changed.</div>';

		};
};

$sql_flagca = mssql_query("Select mvc_flag from ".$mvcore_medb_i." where memb___id = '".$_SESSION['username']."'");
$sql_flag_outa = mssql_fetch_row($sql_flagca);
