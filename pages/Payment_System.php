...
if($corrency_name == '') { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_payment_sys_correntselected.'</div>'; };

if($corrency_name == '1'){
	$cur_n = 'EUR';
	$select_ppx_info= mssql_query("Select pack_name, money, cost_eur from MVCore_PP_Packs where pack_name = '".$pack_name."'"); $s_ppx_i= mssql_fetch_row($select_ppx_info);
} elseif($corrency_name == '2'){
	$cur_n = 'USD';
	$select_ppx_info= mssql_query("Select pack_name, money, cost_usd from MVCore_PP_Packs where pack_name = '".$pack_name."'"); $s_ppx_i= mssql_fetch_row($select_ppx_info);
} elseif($corrency_name == '3'){
	$cur_n = 'GBP';
	$select_ppx_info= mssql_query("Select pack_name, money, cost_gbp from MVCore_PP_Packs where pack_name = '".$pack_name."'"); $s_ppx_i= mssql_fetch_row($select_ppx_info);
};

$select_ppxs_info= mssql_query("Select pack_name from MVCore_PP_Packs where pack_name = '".$pack_name."'"); $s_ppxs_i= mssql_fetch_row($select_ppxs_info);

if($s_ppxs_i[0] == '') { $has_error = '1'; echo'<div class="mvcore-nNote mvcore-nFailure">'.main_p_payment_sys_papalnamenotfoud.'</div>'; };
...
$do_insert = mssql_query("insert into MVCore_Tran_Log (business,currency_code,item_name,custom,invoice,amount,money) VALUES ('".$mvcore['paypal_email']."','".$cur_n."','".$s_ppx_i[2]." ".$cur_n." For ".$s_ppx_i[1]." ".$mvcore['money_name2']."','".$useracc."','".$rand_here."','".$s_ppx_i[2]."','".$s_ppx_i[1]."')");
...
$select_pp_info= mssql_query("Select pack_name, money, cost_eur, cost_usd, cost_gbp from MVCore_PP_Packs order by money asc");
for($i=0;$i < mssql_num_rows($select_pp_info);++$i) {
$s_pp_i= mssql_fetch_row($select_pp_info); 
if($pvsPaymentSystem != 'ok6523') { exit; };
echo'
			<tr>
				<td><b>'.$s_pp_i[0].'</b></td>
				<td><b>'.$s_pp_i[1].'</b></td>
				<td>'.$s_pp_i[2].' EUR <input name="c1" value="'.$s_pp_i[0].'554yy51" type="radio" checked></td>
				<td>'.$s_pp_i[3].' USD <input name="c1" value="'.$s_pp_i[0].'554yy52" type="radio"></td>
				<td>'.$s_pp_i[4].' GBP <input name="c1" value="'.$s_pp_i[0].'554yy53" type="radio"></td>
			</tr>
...
<?php
//Select user Email
$gUEFD = mssql_query("select mail_addr from ".$mvcore_medb_i." where memb___id = '".$_SESSION['username']."'");
$emailIs = mssql_fetch_row($gUEFD);
?>
<div id="div-pw" style="display: none;">
<?php if($mvcore['paymentwall_promo'] != '') { echo''.$mvcore['paymentwall_promo'].'<br>'; }; ?>
...
<?php if($mvcore['superrewards_promo'] != '') { echo''.$mvcore['superrewards_promo'].'<br>'; }; ?>
...
$conv = 'ID_'.$rand_pm_no.'';
$do_insert = mssql_query("insert into MVCore_Tran_Log (business,currency_code,item_name,custom,invoice,amount,money) VALUES ('Interkassa','".$mvcore['interkassa_cur_code']."','Buy Gold Credits Via Interkassa','".$_SESSION['username']."49729".$db_name_drop."','$conv','".$_POST['ik_gew']."','".$cr_value."')");
...
$select_pg_info= mssql_query("Select p_name, money, cost_eur, currency  from MVCore_PG_Packs order by money asc");
for($i=0;$i < mssql_num_rows($select_pg_info);++$i) {
$s_pg_i= mssql_fetch_row($select_pg_info);
...
<form name="pg_frm" method="post" action="https://www.paygol.com/pay" >
	   ...
	   <input type="hidden" name="pg_currency" value="'.$s_pg_i[3].'">
	   ...
	   <input type="hidden" name="pg_price" value="'.$s_pg_i[2].'">
	   <input type="hidden" name="pg_cancel_url" value="">
				<tr>
					<td><b>'.$s_pg_i[1].' '.$mvcore['money_name2'].' For '.$s_pg_i[2].' EUR</b></td>
