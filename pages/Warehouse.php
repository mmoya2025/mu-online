...
$get_char_info_name= mssql_query("Select top 1 name from Character where AccountID='".$useracc."'");
$get_names= mssql_fetch_row($get_char_info_name);
...
$itemtype 		= floor($itt/16);				// Item Category Fix
	...
	switch ($item[$i]) {case $hex: $icok +="1"; $item_now_empty[$i] = $hexlens; break; default: $icok += "0"; $item_now_empty[$i] = $item[$i]; break;};
...
$select_zen_check= mssql_query("Select money from warehouse where accountid='".$useracc."'");
$s_zen_check= mssql_fetch_row($select_zen_check);
...
$s_charwc_i= mssql_fetch_row($select_charwc_info); if($pvsWarehouse != 'ok7844') { exit; };

if($mvcore['guid_column'] == 'memb_guid' || $mvcore['guid_column'] == 'MemberGuid'){ $fix_wcoins_users = $s_charwc_i[0]; } else { $fix_wcoins_users = $useracc; };

//== Error MSG ==//
if($mvcore['ware_affis'] == '1'){

if($ctype == '1'){if($s_c_check[0] < $calc_fee) { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_needmore.' '.$mvcore['money_name1'].' '.main_p_warehouse_payfee.' '.$calc_fee.' '.$mvcore['money_name1'].'.</div>'; };}
elseif($ctype == '2'){if($s_c_check[0] < $calc_fee) { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_needmore.' wcoins '.main_p_warehouse_payfee.' '.$calc_fee.' wcoins.</div>'; };}
elseif($ctype == '3'){if($s_zen_check[0] < $calc_fee) { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_needmore.' zen '.main_p_warehouse_payfee.' '.$calc_fee.' zen.</div>'; };}
elseif($ctype == '4'){if($s_c_check[1] < $calc_fee) { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_needmore.' '.$mvcore['money_name2'].' '.main_p_warehouse_payfee.' '.$calc_fee.' '.$mvcore['money_name2'].'.</div>'; };};
...
if($ctype == '3' && $item_cost >= '1000000001') { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_maxzenval.'</div>'; };
if($ctype == '2' && $item_cost >= '999999') { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_maz.' wcoins '.main_p_warehouse_valuecanbe.'</div>'; };
if($ctype == '1' && $item_cost >= '999999') { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_maz.' '.$mvcore['money_name1'].' '.main_p_warehouse_valuecanbe.'</div>'; };
if($ctype == '4' && $item_cost >= '999999') { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_maz.' '.$mvcore['money_name2'].' '.main_p_warehouse_valuecanbe.'</div>'; };
if($item_cost == '' || $item_cost == ' ' || $item_cost == '0') { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_enteritemcost.'</div>'; };
if(preg_match('/[^0-9]/', $item_cost) === 0) {} else { $has_error = '1'; echo '<div class="mvcore-nNote mvcore-nFailure">'.main_p_warehouse_symbolsnotallowed.'</div>';};
...
$edit_site = mssql_query("UPDATE warehouse SET Items = ".$check_item_new_hex." WHERE AccountId='".$useracc."'");
$edit_site = mssql_query("INSERT INTO MVCore_Market_Items (hex,serial,date,soldby,cost,cate,ptype) VALUES ('".$hex."','".$serials."','".$time_stamp."','".$useracc."','".$item_cost."','".$itemtype."','".$ctype."')");
if($mvcore['ware_affis'] == '1'){
	
	if($ctype == '1'){$edit_site = mssql_query("UPDATE ".$mvcore['credits_table']." SET ".$mvcore['credits_column']." = ".$mvcore['credits_column']." - '".$calc_fee."' WHERE ".$mvcore['user_column']."='".$useracc."'");}
	elseif($ctype == '2'){$edit_site = mssql_query("UPDATE ".$mvcore['wcoins_table']." SET ".$mvcore['wcoins_column']." = ".$mvcore['wcoins_column']." - '".$calc_fee."' WHERE ".$mvcore['guid_column']."='".$fix_wcoins_users."'");}
	elseif($ctype == '3'){$edit_site = mssql_query("UPDATE warehouse SET Money = Money - '".$calc_fee."' WHERE accountid='".$useracc."'"); if($pvsWarehouse != 'ok7844') { exit; }; }
	elseif($ctype == '4'){$edit_site = mssql_query("UPDATE ".$mvcore['credits_table']." SET ".$mvcore['credits2_column']." = ".$mvcore['credits2_column']." - '".$calc_fee."' WHERE ".$mvcore['user_column']."='".$useracc."'");};
...
<?php
	...
	$check_ware_ss = mssql_fetch_row($check_ware);
	if($check_ware_ss[0] == '' || $check_ware_ss[0] == '0x') { echo'<div class="mvcore-nNote mvcore-nInformation">'.main_p_warehouse_wareisempty.'</div>'; } else {
?>
...
$chsadsaata = mssql_query("Select item_name, has_refinery, has_skill, category, id from MVCore_Wshopp where id='".$sy."' and category='".$itemtype."'");
$attrebbradas = mssql_fetch_row($chsadsaata);
