...
if(!extension_loaded('mssql')){
	include('system/error_page.php');
	define("LOG_FILE_MSSQLDIS", "system/engine_logs/mssql_disabled.log"); // Log Save
	...
	mvcore_error('mssql disabled', '<font color="black"><b>Possible solutions</b><br>1) remove ; in php.ini where "extension=php_mssql.dll"</font>');
	exit;
};
if(!extension_loaded('mysql')){
	include('system/error_page.php');
	define("LOG_FILE_MYSQLDIS", "system/engine_logs/mssql_disabled.log"); // Log Save
	...
	mvcore_error('mysql disabled', '<font color="black"><b>Possible solutions</b><br>1) remove ; in php.ini where "extension=php_mssql.dll"</font>');
	exit;
};
...
if($_POST['medb'] == 'yes') { $createstringmesup = ''.$_POST['sql_db'].','.$_POST['sql_medb'].''; } else { $createstringmesup = $_POST['sql_db']; };

$fdbname = explode(',',$createstringmesup);

if(count($fdbname) == 1 && $createstringmesup == '') { $eror_cant_install = 1; echo'<script>$.jGrowl("Fields are empty, fill and try again.", { header: "<font color=red>Error</font>" });</script>'; };
...
for($i=0;$i < count($fdbname);++$i) {
	...
	if($_POST['sql_user'] == '' || $_POST['sql_pass'] == '' || $fdbname[$i] == '' || $_POST['sql_ip'] == '') { } else {
		
		$chup=mssql_connect($_POST['sql_ip'],$_POST['sql_user'],$_POST['sql_pass']);
		$mvcore = mssql_select_db($fdbname[$i],$chup);
...
//-----------------------------------------------------------------------		
		if (isset($_POST['r_reg_form'])){
			...
			if($user_name != ''){
						...
						if(strlen($user_name) >= '11') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - username is to large! max length: 10", { header: "<font color=red>Error</font>" });</script>'; };
						if(strlen($user_pass) >= '11') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - password is to large! max length: 10", { header: "<font color=red>Error</font>" });</script>'; };
						
						if(strlen($user_name) <= '5') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - username is to small! min length: 4", { header: "<font color=red>Error</font>" });</script>'; };
						if(strlen($user_pass) <= '5') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - password is to small! min length: 4", { header: "<font color=red>Error</font>" });</script>'; };
						
						if(strlen($s_answ) >= '21') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - Secret answer is to large! max length: 20", { header: "<font color=red>Error</font>" });</script>'; };
						
						if($e_mail == '') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - You forgot to enter email!", { header: "<font color=red>Error</font>" });</script>'; };
						if($user_pass == '') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - password is missing!", { header: "<font color=red>Error</font>" });</script>'; };
						if($s_ques == '') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - Select secret question from drop down!", { header: "<font color=red>Error</font>" });</script>'; };
						if($s_answ == '') { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - Enter secret answer!", { header: "<font color=red>Error</font>" });</script>'; };
							if(!filter_var($e_mail, FILTER_VALIDATE_EMAIL)) { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - Email is not valid.", { header: "<font color=red>Error</font>" });</script>'; };
						if($user_pass != $rep_pass) { $has_error = '1'; echo'<script>$.jGrowl("Admin Account - Can not register, repeat password did not mach!", { header: "<font color=red>Error</font>" });</script>'; };
						
						if($has_error == '3') { echo'<script>$.jGrowl("Admin Account - Can not register, account username already exists.", { header: "<font color=red>Error</font>" });</script>'; };
						if($has_error == '4') { echo'<script>$.jGrowl("Admin Account - Your email already in use.", { header: "<font color=red>Error</font>" });</script>'; };
					...
					if($has_error >= '1'){} else {
						...
						$do_reg_insert = mssql_query("insert into [".$fdbname[$i]."].[dbo].[AccountCharacter] (id) VALUES ('".$user_name."')");		
