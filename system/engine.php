...
if($mvcore['vote_top_voters'] == '0' || $mvcore['vote_top_voters'] == '') {} else{
	...
	if($Conv_date1 >= $Conv_date2) {
		...
		if($mvcore['reward_sys_vote'] == '1') { $run = mssql_query("Update ".$mvcore['credits_table']." set ".$mvcore['credits_column']." = ".$mvcore['credits_column']." + ".$rewrite_reward." where ".$mvcore['user_column']." IN ( SELECT TOP 10 username COLLATE DATABASE_DEFAULT FROM MVCore_Vote_Log GROUP BY username ORDER BY SUM(votes) DESC )"); }
		elseif($mvcore['reward_sys_vote'] == '2') { $run = mssql_query("Update ".$mvcore['credits_table']." set ".$mvcore['credits2_column']." = ".$mvcore['credits2_column']." + ".$rewrite_reward." where ".$mvcore['user_column']." IN ( SELECT TOP 10 username COLLATE DATABASE_DEFAULT FROM MVCore_Vote_Log GROUP BY username ORDER BY SUM(votes) DESC )"); }
		elseif($mvcore['reward_sys_vote'] == '3') {
			$run = mssql_query("Update ".$mvcore['credits_table']." set ".$mvcore['credits_column']." = ".$mvcore['credits_column']." + ".$rewrite_reward." where ".$mvcore['user_column']." IN ( SELECT TOP 10 username COLLATE DATABASE_DEFAULT FROM MVCore_Vote_Log GROUP BY username ORDER BY SUM(votes) DESC )");
			$run = mssql_query("Update ".$mvcore['credits_table']." set ".$mvcore['credits2_column']." = ".$mvcore['credits2_column']." + ".$rewrite_reward." where ".$mvcore['user_column']." IN ( SELECT TOP 10 username COLLATE DATABASE_DEFAULT FROM MVCore_Vote_Log GROUP BY username ORDER BY SUM(votes) DESC )");		
		};
		//END Special
